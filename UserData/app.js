...
for (var key in usersList) {
  ...
  //creating a button which is getting detail of specific data
  var btn = document.createElement("button");
  btn.setAttribute("onClick", `details(${usersList[key].id})`);
  var btnText = document.createTextNode("Click to view Details");
...
var getDiv = document.getElementById("main-div");
...
function details(el) {
  ...
  for (let key in usersList) {
    if (usersList[key].id === el) {
      for (let key2 in usersList[key]) {
        if (typeof usersList[key][key2] === "object") {
          let para = document.createElement("p");

          for (let key3 in usersList[key][key2]) {
            if (typeof usersList[key][key2][key3] === "object") {
              for (let key4 in usersList[key][key2][key3]) {
                para.appendChild(
                  document.createTextNode(usersList[key][key2][key3][key4])
                );
              }
            } else {
              para.appendChild(
                document.createTextNode(usersList[key][key2][key3])
              );
          ...
          let para = document.createElement("p");
          para.appendChild(document.createTextNode(usersList[key][key2]));
          detailsDiv.appendChild(para);
        }
      }
    }
